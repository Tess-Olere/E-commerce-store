...
export default function PictureProduct({error, data, loading}) {
    ...
    const featuredProducts = data.filter((product) => product.price >=700 && product.price <= 5000)
    console.log(featuredProducts);
  return (
    <Container className= 'mt-5 p-3'>
        <h4 className='mt-5'>Featured Product</h4>
        {loading && <Spinner />}
        ...
        <div className='position-relative'>
            ...
            {featuredProducts.slice(0,12).map((product) =>(
                <div key={product.id} className='me-4'>
                    ...
                    <p className='text-secondary text-sm mb-0'>{FormatCurrency(product.price)}</p>
